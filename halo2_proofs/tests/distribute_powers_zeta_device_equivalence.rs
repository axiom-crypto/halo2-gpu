//! Equivalence tests for the device-backed `extended_to_coeff` path.
//!
//! Host and device execution should produce identical coefficient output.
...
fn run_one(log_n: u32) {
    ...
    let host_out = domain.extended_to_coeff(host_in).expect("host arm failed");
...
#[test]
fn distribute_powers_zeta_device_equivalence_log_n_20() {
    run_one(20);
}

#[test]
#[ignore = "large GPU allocation"]
fn distribute_powers_zeta_device_equivalence_log_n_22() {
    run_one(22);
}

#[test]
#[ignore = "large GPU allocation"]
fn distribute_powers_zeta_device_equivalence_log_n_23() {
    run_one(23);
}
