...
use crate::poly::{Coeff, PolyRef, Polynomial};
        ...
        R: RngCore,
    {
...

        {
            ...
            for commitment_data in poly_map.into_iter() {
                let host_poly = match commitment_data.commitment.poly {
                    PolyRef::Host(p) => p,
                    PolyRef::Device(_) => panic!(
                        "ipa::create_proof does not support Device polynomials; \
                         caller must hold a Host-resident poly before passing to ipa multiopen"
                    ),
                };
                accumulate(commitment_data.set_index, host_poly, Default::default());
            }
        }
