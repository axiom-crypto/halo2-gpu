...
pub trait Params<'params, C: CurveAffine>: Sized + Clone {
    ...
    /// Device-arm of `commit_lagrange`. Operates on a device-resident
    /// Lagrange-basis polynomial directly via a device-pointer MSM (no
    /// scalars H→D).
    ///
    /// Default impl D→H copies and forwards to `commit_lagrange`; the KZG
    /// impl in this crate overrides it to dispatch to the device-scalars +
    /// device-bases MSM kernel.
    fn commit_lagrange_device(
        &self,
        poly: &Polynomial<C::ScalarExt, LagrangeCoeff, crate::poly::Device>,
        r: Blind<C::ScalarExt>,
    ) -> C::CurveExt {
...
pub trait ParamsProver<'params, C: CurveAffine>: Params<'params, C> {
    ...
    /// Device-arm of `commit`. Operates on a device-resident polynomial
    /// directly via a device-pointer MSM (no scalars H→D).
    ///
    /// Default impl D→H copies and forwards to `commit`; the KZG impl in
    /// this crate overrides it to dispatch to the device-scalars +
    /// device-bases MSM kernel.
    fn commit_device(
        &self,
        poly: &Polynomial<C::ScalarExt, Coeff, crate::poly::Device>,
        r: Blind<C::ScalarExt>,
    ) -> C::CurveExt {
        let host = poly.to_host();
        self.commit(&host, r)
