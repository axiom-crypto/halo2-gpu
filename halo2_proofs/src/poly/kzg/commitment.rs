use crate::arithmetic::g_to_lagrange;
use crate::cpu::arithmetic::{best_multiexp_cpu, parallelize};
...
impl<E: Engine> Clone for ParamsKZG<E> {
    fn clone(&self) -> Self {
        Self {
            ...
            g2: self.g2,
            s_g2: self.s_g2,
            g_device: OnceCell::new(),
            g_lagrange_device: OnceCell::new(),
        }
    }
}
    ...
    E::G2Affine: Debug,
{
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        f.debug_struct("ParamsKZG")
            ...
            .field("g_device", &cache_state(&self.g_device))
            .field("g_lagrange_device", &cache_state(&self.g_lagrange_device))
            .finish()
    }
}
...
impl<E: Engine + Debug> ParamsKZG<E> {
    ...
    /// Returns the SRS bases in coefficient form (`[G, sG, s²G, …,
    /// s^{n-1}G]`). Read-only; mutating the SRS would desync the
    /// device-mirror cache (`g_device`).
    pub fn g(&self) -> &[E::G1Affine] {
        &self.g
    }

    /// Returns the SRS bases in Lagrange form (the inverse-DFT of
    /// [`Self::g`]). Read-only; mutating would desync `g_lagrange_device`.
    ...
    fn ensure_g_lagrange_device(&self) -> &DeviceBuffer<E::G1Affine> {
        ...
        assert_eq!(
            d_bases.len(),
            self.g_lagrange.len(),
            "g_lagrange_device cache is stale (host len {} vs device len {}); \
             g_lagrange was mutated outside `downsize` — reconstruct ParamsKZG \
...
// The verifier key currently aliases the prover key; a leaner verifier-key form
// would still need the G1 bases used by the verifier.
    ...
    E::G2Affine: SerdeCurveAffine,
{
    ...
    fn downsize(&mut self, k: u32) {
        ...
        // Invalidate cached device mirrors before mutating `g` / `g_lagrange`
        // — they get recomputed against the new (smaller) sizes on the next
        // commit call.
        self.g_device.take();
        self.g_lagrange_device.take();
    ...
    fn commit_lagrange(&self, poly: &Polynomial<E::Fr, LagrangeCoeff>, _: Blind<E::Fr>) -> E::G1 {
        ...
        if size < GPU_MSM_THRESHOLD {
            return best_multiexp_cpu(poly, &self.g_lagrange[0..size]);
        }
        let d_bases = self.ensure_g_lagrange_device();
        multiexp_gpu_device_bases(poly, d_bases)
            .expect("multiexp_gpu_device_bases failed in commit_lagrange")
    }

    fn commit_lagrange_device(
        &self,
        poly: &Polynomial<E::Fr, LagrangeCoeff, crate::poly::Device>,
        r: Blind<E::Fr>,
    ) -> E::G1 {
        let size = poly.len();
        assert!(self.n() >= size as u64);
        if size < GPU_MSM_THRESHOLD {
            // Small circuits cannot dispatch the GPU MSM kernel; D2H once
            // and fall through to the host-arm CPU multiexp.
            let host = poly.to_host();
            return self.commit_lagrange(&host, r);
        }
    ...
    E::G2Affine: SerdeCurveAffine,
{
    ...
    fn commit(&self, poly: &Polynomial<E::Fr, Coeff>, _: Blind<E::Fr>) -> E::G1 {
        ...
        assert!(self.n() >= size as u64);
        if size < GPU_MSM_THRESHOLD {
            return best_multiexp_cpu(poly, &self.g[0..size]);
        }
        let d_bases = self.ensure_g_device();
        multiexp_gpu_device_bases(poly, d_bases)
            .expect("multiexp_gpu_device_bases failed in commit")
    }
    ...
    fn commit_with_gpu(
        ...
        if size < GPU_MSM_THRESHOLD {
            return best_multiexp_cpu::<E::G1Affine>(poly, &self.g[0..size]);
        }
        let d_bases = self.ensure_g_device();
        multiexp_gpu_device_bases::<E::G1Affine>(poly, d_bases)
            .expect("multiexp_gpu_device_bases failed in commit_with_gpu")
    }

    /// Device-arm of `commit`. Dispatches the device-scalars +
    /// device-bases MSM kernel directly — no scalars H→D.
    fn commit_device(
        &self,
        poly: &Polynomial<E::Fr, Coeff, crate::poly::Device>,
        r: Blind<E::Fr>,
    ) -> E::G1 {
        let size = poly.len();
        assert!(self.n() >= size as u64);
...
mod test {
    ...
    fn test_commit_lagrange() {
        ...
        let b = domain.lagrange_to_coeff(a.clone()).unwrap();
