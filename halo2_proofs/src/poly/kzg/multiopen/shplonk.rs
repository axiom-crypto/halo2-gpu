...
struct IntermediateSets<F: Field + Hash, Q: Query<F>> {
    ...
    super_point_set: HashSet<F, ahash::RandomState>,
}
    ...
    I: IntoIterator<Item = Q> + Clone,
{
    ...
    let mut super_point_set = HashSet::default();
    ...
    // (C_3, {r_2, r_3, r_4}),
    // ...
    let mut commitment_rotation_set_map: Vec<(Q::Commitment, HashSet<F, ahash::RandomState>)> =
        vec![];
    ...
    // `get_eval` may dispatch `cuda.eval_polynomial_device` for
    // device-resident polys; rayon worker threads need the prover's
    // tracing span so the emitted metric carries the parent `group`.
            ...
            let commitments: Vec<Commitment<F, Q::Commitment>> = commitments
                ...
                .map(|commitment| {
                    let evals: Vec<F> = rotations_vec
                        .as_slice()
                        .into_par_iter()
                        .map(|&&rotation| parent_span.in_scope(|| get_eval(commitment, rotation)))
                        .collect();
