    ...
    fn assign_advice<'v>(
        ...
        match to.assign() {
            Ok(to) => {
                ...
                // Skip the cross-phase consistency check: we assign 0s in the first pass
                // of the second phase sometimes, so a previously-assigned value is allowed
                // to be overwritten.
                let val = Arc::new(to);
...
impl<F: FromUniformBytes<64> + Ord> MockProver<F> {
    ...
    pub fn verify_at_rows<I: Clone + Iterator<Item = usize>>(
        ...
        // The "all cells used in instantiated gates have been assigned" check is intentionally
        // disabled: we might query unassigned cells to increase the rotation set size of a gate
        // (for SHPLONK optimization).
        ...
        let selector_errors = self.regions.iter().enumerate().flat_map(|(r_i, r)| {
            r.enabled_selectors.iter().flat_map(move |(selector, at)| {
                ...
                self.cs
                    ...
                    // corresponding gate in some way. Known limitation: leaving a selector
                    // un-enabled does not disable a gate that depends on it, so a gate stays
                    // active if any other queried selector is enabled.
                    .enumerate()
...
mod tests {
    ...
    fn unassigned_cell() {
        ...
        // The mock prover initializes unassigned cells to 0, so this circuit verifies.
        assert_eq!(prover.verify(), Ok(()));
    }
