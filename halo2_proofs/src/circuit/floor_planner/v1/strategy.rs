...
pub fn slot_in_biggest_advice_first(
    ...
    // Sort with `sort_by_cached_key` so that ties between non-unique keys resolve in input
    // order. The input `region_shapes` is sorted by region index (the loop below also relies
    // on this to return `RegionStart`s in the correct order). An unstable sort here would
    // produce output that differs between 32-bit and 64-bit platforms.
