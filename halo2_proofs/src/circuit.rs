...
///
/// The column types here are not constrained to "logical" columns originating from a
/// particular `Chip::Config`; any column the layouter accepts can be referenced.
#[derive(Debug)]
...
impl<'r, F: Field> Region<'r, F> {
    ...
    pub fn assign_advice<'v>(
        ...
        column: Column<Advice>,
        offset: usize,
        // For now only accept Value<F>, later might change to Value<Assigned<F>> for batch inversion
        to: Value<impl Into<Assigned<F>>>,
    ) -> AssignedCell<&'v Assigned<F>, F> {
        self.region
            .assign_advice(column, offset, to.map(|v| v.into()))
    }
