...
impl<'r, F: Field> RegionLayouter<F> for TracingRegion<'r, F> {
    ...
    fn assign_advice<'v>(
        ...
        let _guard = debug_span!("assign_advice", column = ?column, offset = offset).entered();
        debug!(target: "layouter", "Entered");
        self.0.assign_advice(column, offset, to)
    }
    ...
    fn assign_fixed(&mut self, column: Column<Fixed>, offset: usize, to: Assigned<F>) -> Cell {
        let _guard = debug_span!("assign_fixed", column = ?column, offset = offset).entered();
        debug!(target: "layouter", "Entered");
        self.0.assign_fixed(column, offset, to)
    }
...
impl<'cs, F: Field, CS: Assignment<F>> Assignment<F> for TracingAssignment<'cs, F, CS> {
    ...
    fn assign_advice<'v>(
        ...
        if self.in_region {
            ...
            debug!(target: "assign_advice", column = ?column, row = row);
        }
    ...
    fn assign_fixed(&mut self, column: Column<Fixed>, row: usize, to: Assigned<F>) {
        if self.in_region {
            ...
            debug!(target: "assign_fixed", column = ?column, row = row);
        }
